...
//    ============ its for todays_customer_receipt =============
    ...
    public function purchase_report_supplier_wise() {
        $CI = & get_instance();
        $CI->load->library('lreport');
        $this->auth->check_admin_auth();
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Admin_dashboard/purchase_report_supplier_wise/');
        $config["total_rows"] = $this->Reports->purchase_report_supplier_wise_count();
        $config["per_page"] = 50;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        $content = $CI->lreport->purchase_report_supplier_wise($links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
...
//    ========= its for filter_purchase_report_category_wise ==============
    ...
    public function filter_purchase_report_supplier_wise() {
        $CI = & get_instance();
        $CI->load->library('lreport');
        $supplier  = $this->input->post('supplier',TRUE);
        $from_date = $this->input->post('from_date',TRUE);
        $to_date   = $this->input->post('to_date',TRUE);
        $content   = $this->lreport->filter_purchase_report_supplier_wise($supplier, $from_date, $to_date);
        $this->template->full_admin_html_view($content);
    }

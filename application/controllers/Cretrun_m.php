...
class Cretrun_m extends CI_Controller {
    ...
    public function outlet_return_form() {
        ...
        $content = $this->lreturn->outlet_return_form();
    ...
    public function outlet_return() {
        $req_id = $this->input->post('req_id',TRUE);
      //  $invid = $this->db->select('invoice_id')->from('invoice')->where('invoice', $invoice_id)->get()->row();
        $query = $this->db->select('rqsn_id')->from('rqsn')->where('rqsn_id', $req_id)->get();

//        print_r($query->rqsn_id);
//        die();

        if ($query->num_rows() == 0) {
            $this->session->set_userdata(array('error_message' => 'Please input the corrct reacquisition ID'));
            redirect('Cretrun_m/outlet_return_form');
        }
        $CI = & get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->library('lreturn');
        $content = $CI->lreturn->outlet_return_data($req_id);
        $this->template->full_admin_html_view($content);
    }
...
// supplier return form
    ...
    public function return_rqsn_qty() {
        $CI = & get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->model('Returnse');
        $invoice_id = $CI->Returnse->return_rqsn_entry();
    ...
    public function outlet_return_list() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreturn');
        $CI->load->model('Returnse');
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Cretrun_m/outlet_return_list/');
        $config["total_rows"] = $this->Returnse->outlet_return_list_count();
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        #pagination ends
        #
        $content = $this->lreturn->outlet_return_list($links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
...
// date between return report list
    ...
    public function datewise_outlet_return_list() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreturn');
        $CI->load->model('Returnse');
        ...
        $from_date = $this->input->post('from_date',TRUE);
        $to_date = $this->input->post('to_date',TRUE);
        $content = $CI->lreturn->outlet_return_list_datebetween($outlet_id,$from_date, $to_date, $links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
    public function datewise_outlet_wastage_list() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreturn');
        $CI->load->model('Returnse');
        $config["base_url"] = base_url('Cretrun_m/wastage_outlet_return_list/');
        $config["total_rows"] = $this->Returnse->outlet_wastage_return_list_count();
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        $from_date = $this->input->post('from_date',TRUE);
        $to_date = $this->input->post('to_date',TRUE);
        $content = $CI->lreturn->wastage_outlet_return_list_datebetween($outlet_id,$from_date, $to_date, $links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
...
// wastage return list start
    ...
    public function wastage_outlet_return_list() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreturn');
        $CI->load->model('Returnse');
        ...
        #pagination ends
        #
        $content = $this->lreturn->outlet_wastage_return_list($links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
...
// Return delete with invoice id
    ...
    public function delete_return_outlet($return_id = null) {
        $this->load->model('Returnse');
        if ($this->Returnse->returnoutlet_delete($return_id)) {
            #set success message
            $this->session->set_flashdata('message', display('delete_successfully'));
        } else {
            #set exception message
        ...
        redirect("Cretrun_m/outlet_return_list");
    }

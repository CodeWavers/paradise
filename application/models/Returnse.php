...
class Returnse extends CI_Model {
  ...
  $cosdr = array(
    ...
    public function return_rqsn_entry() {
            ...
            for ($i = 0; $i < count($p_id); $i++) {
              ...
              //  $supplier_rate    = $this->supplier_rate($product_id);
               ...
               // $tax              = -$tax_amount[$i];
                ...
                $data1 = array(
                    'rqsn_detail_id'=> $this->generator(15),
                    'rqsn_id'        => $rqsn_id,
                    'product_id'        => $product_id,
                    'a_qty'          => -$product_quantity,
                ...
                $returns = array(
                    ...
                    'ret_qty'       => $product_quantity,
                    'tr_qty'       => $sqty,
                    'date_rqsn' => $this->input->post('invoice_date',TRUE),
                    'date_return'   => $date,
//                    'product_rate'  =>  $product_rate,
//                    'deduction'     => $discount,
//                    'total_deduct'  => $this->input->post('total_discount',TRUE),
...
//                    'net_total_amount' => $this->input->post('grand_total_price',TRUE),
              ...
              //  $this->db->insert('rqsn_details', $data1);
                if ($ads == 1) {
                    $this->db->insert('rqsn_details', $data1);
                }
                $this->db->insert('rqsn_return', $returns);
    ...
    public function outlet_return_list_count() {
        $this->db->select('a.*,c.*,b.product_name');
        $this->db->from('rqsn_return a');
...
///start  return list
    ...
    public function outlet_return_list($perpage, $page) {
        $this->db->select('a.*,c.*,b.product_name');
        $this->db->from('rqsn_return a');
        $this->db->join('product_information b', 'b.product_id = a.product_id');
        $this->db->join('outlet_warehouse c', 'c.outlet_id = a.from_id');
        $this->db->where('usablity', 1);
     //   $this->db->group_by('a.rqsn_id', 'desc');
        $this->db->limit($perpage, $page);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
    ...
    public function return_dateWise_outlet($outlet_id,$from_date, $to_date, $perpage, $page) {
     //   $dateRange = "a.date_return BETWEEN '$from_date' AND '$to_date'";
    ...
    public function wastage_return_dateWise_outlet($outlet_id,$from_date, $to_date, $perpage, $page) {
     //   $dateRange = "a.date_return BETWEEN '$from_date' AND '$to_date'";
...
// date between search return list  supplier/purchase 
    ...
    public function outlet_wastage_return_list_count() {
        $this->db->select('a.*,c.*,b.product_name');
        $this->db->from('rqsn_return a');
        $this->db->join('product_information b', 'b.product_id = a.product_id');
        $this->db->join('outlet_warehouse c', 'c.outlet_id = a.from_id');
...
///start  return list
    ...
    public function outlet_wastage_return_list($perpage, $page) {
        $this->db->select('a.*,c.*,b.product_name');
        $this->db->from('rqsn_return a');
        $this->db->join('product_information b', 'b.product_id = a.product_id');
        $this->db->join('outlet_warehouse c', 'c.outlet_id = a.from_id');
...
// return delete with invoice id  
    ...
    public function returnoutlet_delete($return_id = null) {

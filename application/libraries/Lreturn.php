...
class Lreturn {
    ...
    public function outlet_return_form() {
        $CI = & get_instance();

        $data = array(
            'title' => 'return'
        );
        $returnForm = $CI->parser->parse('return/outlet_return_form', $data, true);
        return $returnForm;
    }
    ...
    public function outlet_return_data($req_id) {
        $CI = & get_instance();
        $CI->load->model('Rqsn');
        $CI->load->model('Web_settings');
        $req_detail = $CI->Rqsn->retrieve_rqsn_editdata($req_id);

//        $to_id=$this->db->select('a.outlet_name')
//            ->from('outlet_warehouse a')
//            ->where('a.outlet_id',$req_detail['to_id'])
//            ->get()->row();

        $i = 0;
        if (!empty($req_detail)) {
            foreach ($req_detail as $k => $v) {
                $i++;
                $req_detail[$k]['sl'] = $i;
            }
        }
        ...
        $data = array(
            'title'         => 'Outlet Return',
            'rqsn_id'    => $req_detail[0]['rqsn_id'],
            'from'=>$req_detail[0]['outlet_name'],
            'from_id'=>$req_detail[0]['from_id'],
            'to_id'=>$req_detail[0]['to_id'],
            'date'=>$req_detail[0]['date'],
               // 'to'=>$to_id->outlet_name,
            'req_all_data' => $req_detail,
...
//        exit();
        $chapterList = $CI->parser->parse('return/outlet_data_form', $data, true);
        return $chapterList;
    }
...
//start  Supplier return form data
    ...
    public function outlet_return_list($links, $perpage, $page) {
        ...
        $outlet_list = $CI->Returnse->outlet_list();
        $return_list = $CI->Returnse->outlet_return_list($perpage, $page);
        if (!empty($return_list)) {
            foreach ($return_list as $k => $v) {
                $return_list[$k]['final_date'] = $CI->occational->dateConvert($return_list[$k]['date_return']);
            }
        ...
        $returnList = $CI->parser->parse('return/outlet_return_list', $data, true);
        return $returnList;
    }
...
// supplier return list
    ...
    public function outlet_wastage_return_list($links, $perpage, $page) {
        ...
        $outlet_list = $CI->Returnse->outlet_list();
        $return_list = $CI->Returnse->outlet_wastage_return_list($perpage, $page);
        if (!empty($return_list)) {
            foreach ($return_list as $k => $v) {
                $return_list[$k]['final_date'] = $CI->occational->dateConvert($return_list[$k]['date_return']);
            }
        ...
        $returnList = $CI->parser->parse('return/wastage_outlet_return_list', $data, true);
        return $returnList;
    }
    ...
    public function outlet_return_list_datebetween($outlet_id,$from_date, $to_date, $links, $perpage, $page) {
        ...
        $CI->load->library('occational');
        $outlet_list = $CI->Returnse->outlet_list();
        $return_list = $CI->Returnse->return_dateWise_outlet($outlet_id,$from_date, $to_date, $perpage, $page);
        if (!empty($return_list)) {
            foreach ($return_list as $k => $v) {
                $return_list[$k]['final_date'] = $CI->occational->dateConvert($return_list[$k]['date_return']);
            }
    ...
    public function wastage_outlet_return_list_datebetween($outlet_id,$from_date, $to_date, $links, $perpage, $page) {
        ...
        $CI->load->library('occational');
        $outlet_list = $CI->Returnse->outlet_list();
        $return_list = $CI->Returnse->wastage_return_dateWise_outlet($outlet_id,$from_date, $to_date, $perpage, $page);
        if (!empty($return_list)) {
            foreach ($return_list as $k => $v) {
                $return_list[$k]['final_date'] = $CI->occational->dateConvert($return_list[$k]['date_return']);
            }
